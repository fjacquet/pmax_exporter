...
// `pmax_exporter_build_info{version="...",goversion="..."} 1`, so a scrape reveals
// exactly which exporter build is running. Standard Prometheus build-info pattern.
func NewBuildInfoCollector(version, goversion string) prometheus.Collector {
	g := prometheus.NewGauge(prometheus.GaugeOpts{
		Namespace:   "pmax_exporter",
		Name:        "build_info",
