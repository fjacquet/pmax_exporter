...
func TestExpandFallback(t *testing.T) {
	unsetForTest(t, "PMAX_FALLBACK_TEST_UNSET")
	t.Setenv("PMAX_FALLBACK_TEST_SET", "real")
	t.Setenv("PMAX_FALLBACK_TEST_EMPTY", "")
	for _, tc := range []struct{ name, in, want string }{
		{"unset falls back", "${PMAX_FALLBACK_TEST_UNSET:-false}", "false"},
		{"set wins over default", "${PMAX_FALLBACK_TEST_SET:-false}", "real"},
		{"exported empty falls back", "${PMAX_FALLBACK_TEST_EMPTY:-other}", "other"},
		{"empty default allowed", "${PMAX_FALLBACK_TEST_UNSET:-}", ""},
		{"mixed with literal text", "a${PMAX_FALLBACK_TEST_UNSET:-b}c", "abc"},
	} {
	...
	if _, err := interpolate("${PMAX_FALLBACK_TEST_UNSET}"); err == nil {
...
func TestExpandSecretRejectsEmpty(t *testing.T) {
	t.Setenv("PMAX_EMPTY_SECRET_TEST", "")

	if _, err := interpolate("${PMAX_EMPTY_SECRET_TEST}"); err != nil {
		t.Fatalf("plain expansion must stay lenient on an exported-empty variable: %v", err)
	}
	if _, err := interpolateSecret("password", "${PMAX_EMPTY_SECRET_TEST}"); err == nil {
