...
- `${VAR:-default}` fallbacks in config env references, ported from `pscale_exporter`.
  ...
  nothing is now rejected, so a stray `PMAX1_PASSWORD=` line fails at startup instead
  of authenticating with an empty credential. The shipped `config.yaml` now uses
  `insecureSkipVerify: "${PMAX1_SKIP_CERTIFICATE:-true}"`, so the setting is env-driven out of the box
